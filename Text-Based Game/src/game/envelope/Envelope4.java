...
/**
 * Envelope4 manages the fourth envelope
 * @author Leonel Fedric B. Tuburan
 *
 */

public class Envelope4 extends Envelope{
	
	/**
	 * Inherits the method in parent class Envelope
	 * Opens and reads msg04
	...
	public void openEnvelope(int[] quantities, boolean[] hasOpened, boolean[] hasFound, boolean[] hasAnswered) {
		try {
			InputStream is = getClass().getResourceAsStream("/letters/msg04.txt");
		...
		if(hasOpened[3] == false) {
			hasOpened[3] = true;
			hasOpened[9] = false;
			ps4.tellStory(quantities, hasOpened, hasFound, hasAnswered);
		}
	}
}
